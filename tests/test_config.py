...
class TestConfigShow:
    def test_show_default_config(self, typer_runner: CliRunner) -> None:
        ...
        with patch("gafour.commands.config.load_config", return_value=Config()):
    ...
    def test_show_reflects_saved_values(self, typer_runner: CliRunner) -> None:
        ...
        with patch("gafour.commands.config.load_config", return_value=cfg):
            result = typer_runner.invoke(app, ["config", "show"])
        assert result.exit_code == 0
        data = json.loads(result.output)
...
class TestConfigSet:
    def test_set_valid_key(self, typer_runner: CliRunner) -> None:
        ...
        with (
            patch("gafour.commands.config.load_config", return_value=Config()),
...
class TestConfigUnset:
    def test_unset_clears_key(self, typer_runner: CliRunner) -> None:
        ...
        with (
            patch("gafour.commands.config.load_config", return_value=cfg),
            patch("gafour.commands.config.save_config") as mock_save,
        ):
